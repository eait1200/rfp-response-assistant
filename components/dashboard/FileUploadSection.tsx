...
function FileItem({ name, progress = 0, status, onRemove }: FileItemProps) {
  ...
  const statusIcons = {
    idle: <FileIcon className="h-5 w-5 text-blue-500" />,
    uploading: <FileIcon className="h-5 w-5 text-blue-500" />,
...
export default function FileUploadSection() {
  const [files, setFiles] = useState<Array<{ name: string; status: UploadStatus; progress: number }>>([
    { name: 'Government_RFP_2023.xlsx', status: 'uploading', progress: 52 },
    { name: 'Healthcare_RFP_2023.xlsx', status: 'success', progress: 100 },
    { name: 'Tech_Services_RFP.xlsx', status: 'error', progress: 78 },
  ]);
    ...
    <Card>
      <CardContent className="p-6">
        <div className="flex flex-col items-center justify-center border-2 border-dashed border-border rounded-lg p-6 text-center">
          <div className="bg-primary-foreground p-3 rounded-full mb-4">
